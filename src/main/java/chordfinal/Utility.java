...
/*
Chord needs some functionality that is not part of the system
-with use of percentages we get a nice view of where each node resides in the circle
-also we can get a nodes port number and address by responding to a simple call
-hashing functions
-any computation for any job
 */
public class Utility {
	...
	public static String hexIdAndPosition (InetSocketAddress addr) {
		...
		return (longTo8DigitHex(hash)+" ("+hash*100/Utility.getPowerOfTwo(32)+"%)");
	}
	...
	public static InetSocketAddress requestAddress (InetSocketAddress server, String req) {
		...
		// using response to create, might fail then and return null
			...
			InetSocketAddress ret = Utility.createSocketAddress(response.split("_")[1]);
			return ret;
		}
	}
	...
	public static String sendRequest(InetSocketAddress server, String req) {
		...
		String response = Utility.inputStreamToString(input);

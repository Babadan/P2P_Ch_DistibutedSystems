...
public class Request_Handler implements Runnable{
	...
	public Socket getTalkSocket() {
		return talkSocket;
	}

	public void setTalkSocket(Socket talkSocket) {
	...
	public Request_Handler(Socket _talkSocket, Chord_Node _local)
	{
	...
	public void run()
	{
		...
		try {
			...
			String request = Utility.inputStreamToString(input);
	...
	private String processRequest(String request)
	{
		...
		if (request.startsWith("CLOSEST")) {
			...
			InetSocketAddress new_pre = Utility.createSocketAddress(request.split("_")[1]);
